import { pgTable, foreignKey, pgEnum, serial, integer, text, timestamp, uniqueIndex, doublePrecision, index, varchar } from "drizzle-orm/pg-core"
  import { sql } from "drizzle-orm"

export const companySize = pgEnum("CompanySize", ['SEED', 'STARTUP', 'SCALEUP', 'MID_SIZE', 'BIG', 'HUGE'])
export const stage = pgEnum("Stage", ['APPLIED', 'ADDED', 'ASK_SYPHTR_TO_REACH_OUT', 'SCREENING', 'SCREENED', 'FIRST_INTERVIEW', 'MID_INTERVIEWS', 'FINAL_INTERVIEW', 'HIRED'])


export const activity = pgTable("Activity", {
	id: serial("id").primaryKey().notNull(),
	profileId: integer("profileId").references(() => profile.id, { onDelete: "set null", onUpdate: "cascade" } ),
	activityStatus: text("activityStatus"),
	link: text("link"),
	title: text("title"),
	sharedRawProfileId: integer("sharedRawProfileId").references(() => sharedRawProfile.id, { onDelete: "set null", onUpdate: "cascade" } ),
});
...
export const candidateStage = pgTable("CandidateStage", {
	id: serial("id").primaryKey().notNull(),
	profileId: integer("profileId").notNull().references(() => profile.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	jobId: integer("jobId").notNull().references(() => job.id, { onDelete: "restrict", onUpdate: "cascade" } ),
...
(table) => {
	return {
		profileIdJobIdKey: uniqueIndex("CandidateStage_profileId_jobId_key").on(table.profileId, table.jobId),
	}
});
...
export const job = pgTable("Job", {
	id: serial("id").primaryKey().notNull(),
	clientId: integer("clientId").references(() => client.id, { onDelete: "set null", onUpdate: "cascade" } ),
...
export const profileOrg = pgTable("ProfileOrg", {
	id: serial("id").primaryKey().notNull(),
	profileId: integer("profileId").notNull().references(() => profile.id, { onDelete: "restrict", onUpdate: "cascade" } ),
...
(table) => {
	return {
		publicIdentifierOrgIdKey: uniqueIndex("ProfileOrg_public_identifier_orgId_key").on(table.orgId, table.publicIdentifier),
	}
});
...
export const product = pgTable("Product", {
	id: serial("id").primaryKey().notNull(),
	name: text("name").notNull(),
	companyId: integer("companyId").notNull().references(() => company.id, { onDelete: "restrict", onUpdate: "cascade" } ),
},
(table) => {
	return {
...
export const profile = pgTable("Profile", {
	...
	talentPoolId: integer("talentPoolId").references(() => talentPool.id, { onDelete: "set null", onUpdate: "cascade" } ),
	orgId: text("orgId"),
	userId: text("userId"),
},
(table) => {
	return {
		publicIdentifierOrgIdKey: uniqueIndex("Profile_public_identifier_orgId_key").on(table.publicIdentifier, table.orgId),
	}
});
...
export const jobCandidates = pgTable("_JobCandidates", {
	a: integer("A").notNull().references(() => job.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	b: integer("B").notNull().references(() => profile.id, { onDelete: "cascade", onUpdate: "cascade" } ),
},
(table) => {
	return {
...
export const sensitiveProfileData = pgTable("SensitiveProfileData", {
	id: serial("id").primaryKey().notNull(),
	email: text("email"),
	phone: text("phone"),
	cv: text("cv"),
...
(table) => {
	return {
		profileIdUserIdKey: uniqueIndex("SensitiveProfileData_profileId_userId_key").on(table.profileId, table.userId),
	}
});
...
export const sharedRawProfile = pgTable("SharedRawProfile", {
	id: serial("id").primaryKey().notNull(),
	publicIdentifier: text("public_identifier"),
	linkedinProfileUrl: text("linkedin_profile_url").notNull(),
	lastUpdated: timestamp("last_updated", { precision: 3, mode: 'string' }),
...
(table) => {
	return {
		publicIdentifierKey: uniqueIndex("SharedRawProfile_public_identifier_key").on(table.publicIdentifier),
	}
});

export const companyProductCategory = pgTable("CompanyProductCategory", {
	id: serial("id").primaryKey().notNull(),
	companyId: integer("companyId").notNull().references(() => company.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	productId: integer("productId").notNull().references(() => product.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	categoryId: integer("categoryId").notNull().references(() => category.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	percentage: integer("percentage").notNull(),
},
(table) => {
	return {
		companyIdProductIdCategoryIdKey: uniqueIndex("CompanyProductCategory_companyId_productId_categoryId_key").on(table.companyId, table.productId, table.categoryId),
	}
});